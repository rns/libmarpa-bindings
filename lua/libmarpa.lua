...
codes = { errors = errors, events = events }
...
--[[
  if failure code exists, check result against it
  elseif informational code exists
  if success code exists
...
--  pti("assert_result: ", result, func, object, eec[func])
  if eec[func][FAILURE] ~= nil then
    ...
    if eec[func][FAILURE] == "ffi.NULL" then
      ...
      assert( result ~= eec[func][FAILURE], error_string(result, func, object) )
    end
  end
  if eec[func][INFORMATION] ~= nil then
    assert( result ~= eec[func][INFORMATION], error_string(result, func, object))
  end
