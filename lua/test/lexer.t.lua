...
for _, opts in ipairs{ { 'lua', ts_lua_patterns }, { 'pcre', ts_pcre } } do

  local opt_patterns, ts = unpack(opts)

  local lex = lexer.new{ tokens = ts, input = input, patterns = opt_patterns }
  ...
  if lex ~= nil then
    ...
    local i = 1
    local expected_terminals = {} for _, v in pairs(ts) do expected_terminals[v[2]] = 1 end
    note("tokenizing\n" .. input)
    while true do
    ...
    note("tokenized with " .. opt_patterns .. " patterns")
  else
    note("no lexer for " .. opt_patterns .. " patterns")
  end
